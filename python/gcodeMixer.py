...
startFileGlob=argFileDir + "gcodeMixer.*.txt"
...
class MainApp(tk.Frame):
    ...
    def create_main(self):
        ...
        self.w = tk.Label(self, text="GCode Mixer",font=("Helvetica", 40))
        self.w.pack()
        ...
        for file in sorted(os.listdir(directory), key=alphanum_key):
            ...
            if fileName.endswith(".gcode") and "_COMPILED_" in fileName:
                try:
                    ...
                    print("ERROR: Couldn't clean up compiled dir " + gcodeDir)
    ...
    def generateGcode(self, gcodeDir, title):
        ...
        with open(outFileName,"a") as outFile:
            ...
            for gcode in gcodeFooter:
                outFile.write(gcode + '\n')
        self.gcodeCompilePath=outFileName
        self.successCompile=True

        if selected == False:
            messagebox.showerror("Error","No gcode files selected")
            return
        tk.Label(self, text="GCode Mixed!",font=("Helvetica", 18), fg="red").pack(side="top")
        tk.Label(self, text=outFileName.replace(rootdir, ""),font=("Helvetica", 10), fg="red").pack(side="top")


while True:
    fileExists=False 
    argFiles = list(glob.glob(startFileGlob))
    ...
    if fileExists or DEBUG:
        ...
        app.mainloop()
        if app.successCompile:
            print("Successfull compile: " + app.gcodeCompilePath)
            with open(doneFile,"a") as doneFile:
                doneFile.write(app.gcodeCompilePath)
        else:
        ...
        # for file in argFiles:
        #     filename = os.fsdecode(file)
        #     curDoneFile=filename+".DONE"
        #     if os.path.exists(curDoneFile) and doneFile != curDoneFile:
        #         # Remove both these files, they are done
        #         os.remove(filename)
        #         os.remove(curDoneFile)
